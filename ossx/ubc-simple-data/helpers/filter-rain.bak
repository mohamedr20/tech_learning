...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname filter-rain) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
;; Move the drops down (+ SPEED (drop-y d))
...
;<use example from ListOfDrop data definition>
...
;; Drop -> Drop
;; increase a drop's y value by the speed

;(define (tick-drop (make-drop 12 21)) empty)
...
;; ListOfDrop -> ListOfDrop
;; filter out drops that leave the screen, where a drop's y is greater than the height of the screen

;(define (filter-drops lod) empty)

(check-expect (filter-drops empty) empty)
(check-expect (filter-drops (cons (make-drop 3 4)
...
;; Drop -> Boolean
;; Determine if the drop-y is greater than the height of the screen

;(define (onscreen? (make-drop 40 (+ 5 HEIGHT))) false)
...
;; Drop Image -> Image
;; place drops on img as specified by d
