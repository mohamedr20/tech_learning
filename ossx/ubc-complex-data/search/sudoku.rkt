...
;; Produces a solution for a board or false if the board is unsolvable
...
;; Asssume: board is valid, so it is solved if it is full
...
;; Board -> Pos
;; produces the positon of the first blank square
...
;(r-c->pos 0 5) ;; This function takes in a row and column number and produces a Position[0,80]

(define (read-square bd p)
  (list-ref bd p)) ; (list-ref BD2 , position of 5) = the value on the board is 6              
...
;; Given a board, a positon and a new value, return a new board with the new value having replaced the old one
