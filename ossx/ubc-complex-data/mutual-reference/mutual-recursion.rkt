...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname mutual-recursion) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
(define (sum-data--loe loe)
  (cond [(empty? loe) 0]
        [else
         (+ (sum-data--element (first loe)) (sum-data--loe (rest loe)))]))
...
(check-expect (collect-name--loe empty) empty)
(check-expect (collect-name--element F1) (list "F1"))
(check-expect (collect-name--element F2) (list "F2"))
(check-expect (collect-name--element F3) (list "F3"))
(check-expect (collect-name--element D4) (list "D4" "F1" "F2"))
(check-expect (collect-name--element D5) (list "D5" "F3"))
(check-expect (collect-name--element D6) (list "D6" "D4" "F1" "F2" "D5" "F3"))

(define (collect-name--element e)
  (cons (elt-name e)    ;String
        (collect-name--loe (elt-subs e))))

(define (collect-name--loe loe)
  (cond [(empty? loe) empty]
        [else
         (append (collect-name--element (first loe))
                 (collect-name--loe (rest loe)))])) 
